...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white border border-slate-200/60 rounded-2xl p-5 flex flex-col justify-between hover:border-blue-200 hover:shadow-xl hover:shadow-blue-50/20 transition-all duration-300 group">
            ...
            <div class="mt-6 pt-4 border-t border-slate-100 flex items-center justify-between gap-4">
                ...
                <div class="flex items-center gap-2">
                    <a href="{{ route('campaigns.edit', $item->id) }}" class="p-2 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-all" title="Edit Data">
                        <i class="fa-solid fa-pen-to-square text-sm"></i>
                    </a>
                    
                    <form action="{{ route('campaigns.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus barang donasi ini?')" class="inline">
                        @csrf
                        @method('DELETE')
    ...
    @empty
        <div class="col-span-full bg-white border border-slate-200 border-dashed rounded-2xl p-12 text-center">
            ...
            <a href="{{ route('campaigns.create') }}" class="mt-4 inline-flex items-center gap-2 text-xs font-semibold text-blue-600 hover:text-blue-700 transition-colors">
